...
window['ZuckitaDaGalera'] = window['Zuck'] = function(timeline, options) {
  ...
  var q = function(query) {
    ...
    timeAgo = function(date_str) {
      ...
      var time_formats = [
        [60, lang['segundos'], 1], // 60
        [120, '1' + lang['minuto'], ''], // 60*2
        [3600, lang['minutos'], 60], // 60*60, 60
        [7200, '1' + lang['hora'], ''], // 60*60*2
        [86400, lang['horas'], 3600], // 60*60*24, 60*60
        [172800, lang[' ontem'], ''], // 60*60*24*2
        [604800, lang['dias'], 86400]
      ];
      ...
      var segundos = (new Date - new Date(time)) / 1000;
      var token = lang['atras'],
        list_choice = 1;
      if (segundos < 0) {
      ...
      while (format = time_formats[i++]) {
        if (segundos < format[0]) {
          if (typeof format[2] == 'string') {
            return format[list_choice];
          } else {
            return Math.floor(segundos / format[2]) + ' ' + format[1] + ' ' + token;
          }
        }
      }
